...
public class WorldCommandProcessor extends CommandProcessor {
    ...
    public void setBlock(int x, int y, int z, Material material) {
        plugin.getServer().getWorlds().getFirst().getBlockAt(x, y, z).setType(material);
    }
    ...
    public Material getBlock(int x, int y, int z) {
        return plugin.getServer().getWorlds().getFirst().getBlockAt(x, y, z).getType();
    }
        ...
        for (int x = Math.min(x1, x2); x <= Math.max(x1, x2); x++) {
            for (int y = Math.min(y1, y2); y <= Math.max(y1, y2); y++) {
                for (int z = Math.min(z1, z2); z <= Math.max(z1, z2); z++) {
                    operator.op(plugin.getServer().getWorlds().getFirst().getBlockAt(x, y, z));
                }
            }
        }
    }
